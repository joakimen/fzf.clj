...
(comment
  ...
  ;; Example 6: Action with :in-process-fn (in-process reload)
  ...
  (let [app-data (atom ["initial-a" "initial-b"])]
    (fzf/fzf {:header {:header-str "Press F6 to reload (bbnc)"}
              ...
              {"f6" [{:action-name "reload"
                      :in-process-fn (fn [current-query & selections]
                                        (swap! app-data conj (str "new-item-" (count @app-data)))
                                        (println (str "in-process-fn: Current query '" current-query "', new data: " @app-data))
  ...
  ;; Example 10: Action with :in-process-fn (for execute)
  ;; Press F10 to execute an in-process Clojure function with the current query and selection.
  ;; This function can access application state. Its return value is the "output" of the execute action,
  ;; which is then used here to update the header.
  (let [app-counter (atom 0)]
    (fzf/fzf {:header {:header-str "Press F10 to exec in-process (toggles item, updates the atom in scope)"}
              :multi true
              :command-bindings
              {"f10" ["toggle"
                      {:action-name "execute-silent"
                       :in-process-fn (fn [query selections-str] ; query from {q}, selection-str from {+}
                                        (let [new-count (swap! app-counter inc)
                                              selection-items (str/split selections-str #" ")]
                                          (println (format "In-process execute: query='%s', selection(s)='%s', counter=%s",
                                                           query,
                                                           selection-items,
                                                           new-count))
                                          (println (str "Counter: " new-count ", All Sels: " selections-str))
                                          ; (out) In-process execute: query='', selection(s)='alpha', counter=1
                                          ; (out) Counter: 1, All Sels: alpha
                                          ; (out) In-process execute: query='', selection(s)='alpha beta', counter=2
                                          ; (out) Counter: 2, All Sels: alpha beta
                                          ;; with execute-silent we're not interested in the output
                                          selections-str))}]}}
             sample-input)))
